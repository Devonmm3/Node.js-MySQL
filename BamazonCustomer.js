            ...
            var getInventory = function () {
                connection.query("SELECT * FROM Products WHERE ItemID =" + shopperIdChoice, function (err, result) {
                    ...
                    if (userAttemptsBuy > productInventory || productInventory === 0) {
                        ...
                        connection.query("UPDATE Products SET StockQuantity = " + hasInStock + " WHERE ProductID =" + shopperIdChoice, function (err, result) {
                            if (err) throw err;
                            connection.query("SELECT ProductID, ProductName, DepartmentName, Price, StockQuantity FROM products WHERE ProductID =" + shopperIdChoice, function (err, result) {
                                console.log(result);
                            });
